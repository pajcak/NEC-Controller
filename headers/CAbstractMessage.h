...
public:
    ...
    virtual int                               getLength(unsigned char & hi, unsigned char & lo) = 0;
    virtual std::basic_string<unsigned char>  getLength() = 0;
    // saves all bytes to _outBuffer and returns the number of bytes saved
    virtual std::basic_string<unsigned char>  getBuffer() = 0;
protected:
